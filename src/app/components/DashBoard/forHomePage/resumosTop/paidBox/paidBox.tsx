...
import DocumentIcon from "@/assets/svg/forHomeDashboard/documentIcon";
import { useRouter } from "next/navigation";
import actionTypes from "@/redux/reducers/action-types";

export default function PaidBox() {
  const { paidBills }: { paidBills: BillType[] } = useSelector(
    ...
    <>
      ...
      >
        ...
        >
          ...
          <Flex flexDirection="column">
            ...
            <Text
              ...
              color="#343447"
            >
              {formatterCoin(SumAllBillsValue(paidBills))}
            </Text>
          </Flex>
        </Flex>
        <Flex
        ...
        >
          ...
          <Box bg="#EEF6F6" borderRadius="0.5rem" padding="1% 8%">
            <Text
              fontFamily="Montserrat"
              fontSize="0.8rem"
              fontStyle="normal"
        ...
        <Box height="52%" width="100%" bg="#fff">
          ...
          {paidBills.map((item, index: number) => {
            if (index <= 3) {
              return (
                <RowItem
                  key={item.id}
        ...
        >
          <Text
            ...
            onClick={() => {
              dispatch({
                type: actionTypes.seeAllpaidBillsIsActiveKey,
                payload: true,
              });
